...
class LifeLone_MNIST(object):
    ...
    def Create_VAEs(self,index,selectedComponentIndex):
        ...
        if self.componentCount == 0:
            # Classifier
            net = ResNet([2, 2, 2, 2], 10)
            net.compile(tf.keras.optimizers.SGD(self.classifier_learningRate,momentum=0.9, decay=5e-4),
                        loss=tf2.losses.CategoricalCrossentropy(from_logits=True),
                        metrics=['accuracy'])
            ...
            reco = Generator_SVHN_Sigmoid(SubGeneratorStr, continous_variables1_super, batch_size=64,
                                          reuse=False)

            feature = 0
            # generation
            reco2 = Generator_SVHN_Sigmoid(SubGeneratorStr, self.z, batch_size=64,
                                           reuse=True)
            ...
            # Classifier
            net = ResNet([2, 2, 2, 2], 10)
            net.compile(tf.keras.optimizers.SGD(self.learning_rate,momentum=0.9, decay=5e-4),
                        loss=tf2.losses.CategoricalCrossentropy(from_logits=True),
                        metrics=['accuracy'])
            self.classifierModelArr.append(net)
    ...
    def Calculate_Accuracy_ByALL_Single_ByDataAugument(self, testX,testY, index):
        totalCount = np.shape(testX)[0]
        myPro = []
    ...
    def Calculate_Accuracy_ByALL_Single(self, testX,testY, index):
        totalCount = np.shape(testX)[0]
        myPro = []
    ...
    def train(self):
        ...
        with tf.Session(config=config) as sess:
            ...
            acc2 = self.Calculate_Accuracy_ByALL_Single_ByNoise(self.data_textX,self.data_textY,0)
            print("single acc")
            print(acc2)

            acc2 = self.Calculate_Accuracy_ByALL_Single(self.data_textX,self.data_textY,0)
            print("tt acc")
            print(acc2)


            acc2 = self.Calculate_Accuracy_ByALL_Single_ByDataAugument(self.data_textX,self.data_textY,0)
            print("tt acc")
            print(acc2)
