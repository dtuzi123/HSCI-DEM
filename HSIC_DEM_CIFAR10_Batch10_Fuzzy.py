...
def Exchange(set1,label1,set2,label2):
    count1 = np.shape(set1)[0]
    count1 = int(count1 / 2)

    count2 = np.shape(set2)[0]
    count2 = int(count2 / 2)

    set1_1 = set1[0:count1]
    label1_1 = label1[0:count1]

    set2_1 = set2[0:count2]
    label2_1 = label2[0:count2]

    set1_1 = np.concatenate((set1_1,set2_1),axis=0)
    label1_1 = np.concatenate((label1_1,label2_1),axis=0)

    set2_1 = np.concatenate((set2_1,set1[count1 :np.shape(set1)[0] ]))
    label2_1 = np.concatenate((label2_1,label1[count1 :np.shape(label1)[0] ]))

    return set1_1,label1_1,set2_1,label2_1
...
class LifeLone_MNIST(object):
    def __init__(self):
        ...
        self.arr1, self.labelArr1, self.arr2, self.labelArr2 = Exchange(self.arr1,self.labelArr1,self.arr2,self.labelArr2)
        self.arr3, self.labelArr3, self.arr4, self.labelArr4 = Exchange(self.arr3,self.labelArr3,self.arr4,self.labelArr4)
    ...
    def train(self):
        ...
        with tf.Session(config=config) as sess:
            ...
            myThirdName = "results/RealClass_CIFAR10.txt"
            ...
            myThirdName = "results/ComponentCount_CIFAR10.txt"
            f = open(myThirdName, "w", encoding="utf-8")
            for i in range(np.shape(arr1)[0]):
                f.writelines(arr1[i])
                f.writelines('\n')
