...
class LifeLone_MNIST(object):
    ...
    def Create_VAEs(self,index,selectedComponentIndex):
        ...
        if self.componentCount == 0:
            ...
            KL_divergence1 = 0.5 * tf.reduce_sum(
                tf.square(z_mean_super) + tf.square(z_log_sigma_super) - tf.log(1e-8 + tf.square(z_log_sigma_super)) - 1, 1)
    ...
    def SelectComponentBySingleNoise(self,single):
        ...
        for i in range(self.batch_size):
            a1 = np.random.normal(0.0, 0.01, size=(32, 32, 3))
            b = single + a1
            myarr.append(b)
    ...
    def Calculate_Accuracy_ByALL_Single(self, testX,testY, index):
        ...
        for i in range(totalBatchCount):
            single = testX[i]
            index = self.SelectComponentBySingle(single)
            pred = self.classifierModelArr[index].predict(batch)
            pred = np.argmax(pred,axis=1)
            myPro.append(pred[0])
            #for j in range(self.batch_size):
