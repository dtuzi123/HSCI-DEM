...
class LifeLone_MNIST(object):
    def __init__(self):
        ...
        self.cifar_train_x = self.cifar_train_x / 127.5 - 1
        self.cifar_test_x = self.cifar_test_x / 127.5 - 1

        self.cifar_train_x_sigmoid, self.cifar_train_label_sigmoid, self.cifar_test_x_sigmoid, self.cifar_test_label_sigmoid = prepare_data()
        self.cifar_train_x_sigmoid = self.cifar_train_x_sigmoid / 255.0
        self.cifar_test_x_sigmoid = self.cifar_test_x_sigmoid / 255.0
        ...
        self.arr1_sigmoid, self.labelArr1_sigmoid, self.arr2_sigmoid, self.labelArr2_sigmoid, self.arr3_sigmoid, self.labelArr3_sigmoid, self.arr4_sigmoid, self.labelArr4_sigmoid, self.arr5_sigmoid, self.labelArr5_sigmoid = Split_dataset_by5(
            x_train,
            y_train)

        self.arr1_test_sigmoid, self.labelArr1_test_sigmoid, self.arr2_test_sigmoid, self.labelArr2_test_sigmoid, self.arr3_test_sigmoid, self.labelArr3_test_sigmoid, self.arr4_test_sigmoid, self.labelArr4_test_sigmoid, self.arr5_test_sigmoid, self.labelArr5_test_sigmoid = Split_dataset_by5(
            x_test,
            y_test)
        ...
        totalSet_sigmoid = np.concatenate((self.arr1_sigmoid, self.arr2_sigmoid, self.arr3_sigmoid, self.arr4_sigmoid, self.arr5_sigmoid),
                                  axis=0)

        self.totalSet = totalSet
        self.totalSet_sigmoid = totalSet_sigmoid

        totalTest = np.concatenate((self.arr1_test, self.arr2_test, self.arr3_test, self.arr4_test, self.arr5_test),
                                   axis=0)

        totalTest_sigmoid = np.concatenate((self.arr1_test_sigmoid, self.arr2_test_sigmoid, self.arr3_test_sigmoid, self.arr4_test_sigmoid, self.arr5_test_sigmoid),
                                   axis=0)

        self.totalTestingSet = totalTest
        self.totalTestingSet_sigmoid = totalTest_sigmoid

        self.data_textX_sigmoid = np.concatenate((self.arr1_test_sigmoid,self.arr2_test_sigmoid,self.arr3_test_sigmoid,self.arr4_test_sigmoid,self.arr5_test_sigmoid),axis=0)
    ...
    def Create_VAEs(self,index,selectedComponentIndex):
        ...
        if self.componentCount == 0:
            ...
            z_mean_super, z_log_sigma_super = TotalEncoder32(self.inputs, SubEncoder, batch_size=64,
                                                             reuse=False)
            ...
            reco = Generator_SVHN_Tanh(SubGeneratorStr, continous_variables1_super, batch_size=self.batch_size,
                                          reuse=False)

            feature = 0
            # generation
            reco2 = Generator_SVHN_Tanh(SubGeneratorStr, self.z, batch_size=self.batch_size,
                                           reuse=True)
            ...
            KL_divergence1 = 0.5 * tf.reduce_sum(
                tf.square(z_mean_super) + tf.square(z_log_sigma_super) - tf.log(
                    1e-8 + tf.square(z_log_sigma_super)) - 1, 1)
            ...
            # Building the first component
            z_mean_super, z_log_sigma_super = TotalEncoder32(self.inputs, SubEncoder, batch_size=self.batch_size,
                                                             reuse=False)
            # z_mean_super = CIFAR_SuperEncoder(self.inputs, SuperEncoder, is_training, batch_size=64, reuse=False)
            continous_variables1_super = z_mean_super + z_log_sigma_super * tf.random_normal(tf.shape(z_mean_super), 0,
                                                                                             1, dtype=tf.float32)
    ...
    def build_model(self):
        ...
        reconstruction_loss1 = tf.reduce_mean(tf.reduce_sum(tf.square(self.KDinputs - self.inputs), [1, 2, 3]))
        self.totalRecoLoss = reconstruction_loss1
    ...
    def SelectComponentBySingle(self,batch):
        losses = []
        for i in range(np.shape(self.lossArr)[0]):
            loss1 = self.sess.run(self.lossArr[i], feed_dict={self.inputs: batch})
            losses.append(loss1)
    ...
    def SelectComponentBySingleNoise(self,single):
        x_test = np.tile(single,(self.batch_size,1,1,1))

        myarr = []
        for i in range(self.batch_size):
            a1 = np.random.normal(0.0, 0.1, size=(32, 32, 3))
            b = single + a1
            myarr.append(b)
        ...
        lossArr = []
        for i in range(self.componentCount):
            loss = self.sess.run(self.lossArr[i],feed_dict={self.inputs:myarr})
            lossArr.append(loss)
        index = np.argmin(lossArr)
        return x_test,index

    def SelectComponentBySingleDataAugument(self,single):
        x_test = np.tile(single,(self.batch_size,1,1,1))

        myarr = []
        for i in range(np.shape(self.GeneratorArr)[0]):
            batch_z = np.random.uniform(-1, 1, [self.batch_size, self.z_dim]).astype(np.float32)
            aa1 = self.sess.run(self.GeneratorArr[i],feed_dict={self.z:batch_z})
            loss1 = self.sess.run(self.totalRecoLoss,feed_dict={self.inputs:aa1,self.KDinputs:x_test})
            myarr.append(loss1)

        index = np.argmin(myarr)
        return x_test,index
    ...
    def Calculate_Accuracy_ByIndex(self, testX,testY, testX_sigmoid):

        p1 = int(np.shape(testX)[0] / self.batch_size)
        myPro = self.Predictions_By_Index2(testX_sigmoid,index)
        target = [np.argmax(one_hot)for one_hot in testY]
        sumError = 0
        accCount = 0
        for i in range(np.shape(myPro)[0]):
    ...
    def Calculate_Accuracy_ByALL_Batch(self, testX,testY, textX_sigmoid):
        totalCount = np.shape(testX)[0]
        myPro = []
        ...
        for i in range(totalBatchCount):
            ...
            batch_sigmoid = textX_sigmoid[i*self.batch_size:(i+1)*self.batch_size]

            index = self.SelectComponentByBatch(batch)
            pred = self.classifierModelArr[index].predict(batch_sigmoid)
            pred = np.argmax(pred,axis=1)
            for j in range(self.batch_size):
                myPro.append(pred[j])
    ...
    def Calculate_Accuracy_ByALL_Single_ByNoise(self, testX,testY, index):
        totalCount = np.shape(testX)[0]
        myPro = []

        totalBatchCount = totalCount#int(totalCount / self.batch_size)
        for i in range(totalBatchCount):
            single = testX[i]
            batch,index = self.SelectComponentBySingleNoise(single)
    ...
    def Calculate_Accuracy_ByALL_Single_ByDataAugument(self, testX,testY, testX_sigmoid):
        totalCount = np.shape(testX)[0]
        myPro = []
        ...
        for i in range(totalBatchCount):
            single = testX[i]
            single_sigmoid = testX_sigmoid[i]
            batch_sigmoid = self.ReturnBatch(single_sigmoid)

            batch,index = self.SelectComponentBySingleDataAugument(single)
            pred = self.classifierModelArr[index].predict(batch_sigmoid)
            pred = np.argmax(pred,axis=1)
            myPro.append(pred[0])
            #for j in range(self.batch_size):
    ...
    def ReturnBatch(self,single):
        x_test = np.tile(single,(self.batch_size,1,1,1))
        return x_test


    def Calculate_Accuracy_ByALL_Single(self, testX,testY, testX_sigmoid):
        totalCount = np.shape(testX)[0]
        myPro = []
        ...
        for i in range(totalBatchCount):
            ...
            batch,index = self.SelectComponentBySingle(single)
            pred = self.classifierModelArr[index].predict(batch_sigmoid)
            pred = np.argmax(pred,axis=1)
            myPro.append(pred[0])
            #for j in range(self.batch_size):
    ...
    def train(self):
        ...
        with tf.Session(config=config) as sess:
            ...
            self.FixedMemory_sigmoid = self.totalSet_sigmoid[0:self.batch_size]

            self.FixedMemoryLabel = self.totalLabel[0:self.batch_size]
            self.FixedMemory = np.array(self.FixedMemory)
            self.FixedMemory_sigmoid = np.array(self.FixedMemory_sigmoid)
            ...
            for t1 in range(int(np.shape(self.totalSet)[0] / self.data_stream_batch)):
                ...
                newX_sigmoid = self.totalSet_sigmoid[t1 * self.data_stream_batch:(t1 + 1) * self.data_stream_batch]

                self.keepcount = self.keepcount + 1
                ...
                if np.shape(self.FixedMemory)[0] == 0:
                    ...
                    self.FixedMemory_sigmoid = newX_sigmoid
                else:
                    ...
                    self.FixedMemory_sigmoid = np.concatenate((self.FixedMemory_sigmoid,newX_sigmoid),axis=0)
                ...
                if self.componentCount == 1:
                    ...
                    for epoch in range(epochs):
                        ...
                        Xtrain_binarized_sigmoid = self.FixedMemory_sigmoid

                        n_examples = np.shape(Xtrain_binarized)[0]
                        index2 = [i for i in range(n_examples)]
                        np.random.shuffle(index2)
                        Xtrain_binarized = Xtrain_binarized[index2]
                        Xtrain_binarized_sigmoid = Xtrain_binarized_sigmoid[index2]
                        ...
                        for idx in range(myCount):
                            ...
                            batchImages_sigmoid = Xtrain_binarized_sigmoid[idx * self.batch_size:(idx + 1) * self.batch_size]
                            ...
                            feed_dict = {self.inputs: batchImages, self.label:batchLabels})
                            '''
                    self.classifierModelArr[self.componentCount-1].fit(self.FixedMemory_sigmoid, self.FixedMemoryLabel,
                                                                                          verbose=0, epochs=self.classifier_epoches,
                                                                                          validation_freq=1,
                                                                                          shuffle=True)
                    self.classifierModelArr[self.componentCount-2].fit(self.FixedMemory_sigmoid, self.FixedMemoryLabel,
                                                                                          verbose=0, epochs=self.classifier_epoches,
                                                                                          validation_freq=1,
                                                                                          shuffle=True)
                else:
                ...
                if np.shape(self.FixedMemory)[0] > self.maxMmeorySize:
                    ...
                    if self.componentCount == 1:
                        ...
                        if self.keepcount > 1000:
                            ...
                            self.FixedMemory_sigmoid = self.FixedMemory_sigmoid[self.batch_size:myCount3]
            ...
            acc = self.Calculate_Accuracy_ByALL_Batch(self.data_textX,self.data_textY,self.data_textX_sigmoid)

            print("other acc")
            print(acc)

            acc2 = self.Calculate_Accuracy_ByALL_Single(self.data_textX,self.data_textY,self.data_textX_sigmoid)
            print("tt acc")
            print(acc2)

            acc3 = self.Calculate_Accuracy_ByALL_Single_ByDataAugument(self.data_textX,self.data_textY,self.data_textX_sigmoid)
            print("aaaa acc")
            print(acc2)
