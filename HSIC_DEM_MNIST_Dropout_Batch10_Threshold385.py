...
class LifeLone_MNIST(object):
    ...
    def train(self):
        ...
        with tf.Session(config=config) as sess:
                ...
                if np.shape(self.FixedMemory)[0] > self.maxMmeorySize:
                    ...
                    if self.componentCount == 1:
                        ...
                        for kk in range(self.componentCount - 1):
                            ...
                            hsic, _ = hsic_gam(code1Arr, code2Arr)
                            dhsicArr.append(hsic)
            ...
            realarr5 = []
            for i in range(self.batch_size):
                single1 = self.arr1_test[i]
                single1 = np.reshape(single1, (1, -1))
                x1, index1 = self.SelectComponentBySingle(single1)
                reco1 = self.sess.run(self.recoArr[index1], feed_dict={self.inputs: x1})
                arr1.append(reco1[0])
                realarr1.append(x1[0])

                single2 = self.arr2_test[i]
                single2 = np.reshape(single2, (1, -1))
                x2, index2 = self.SelectComponentBySingle(single2)
                reco2 = self.sess.run(self.recoArr[index2], feed_dict={self.inputs: x2})
                arr2.append(reco2[0])
                realarr2.append(x2[0])

                single3 = self.arr3_test[i]
                single3 = np.reshape(single3, (1, -1))
                x3, index3 = self.SelectComponentBySingle(single3)
                reco3 = self.sess.run(self.recoArr[index3], feed_dict={self.inputs: x3})
                arr3.append(reco3[0])
                realarr3.append(x3[0])

                single4 = self.arr4_test[i]
                single4 = np.reshape(single4, (1, -1))
                x4, index4 = self.SelectComponentBySingle(single4)
                reco4 = self.sess.run(self.recoArr[index4], feed_dict={self.inputs: x4})
                arr4.append(reco4[0])
                realarr4.append(x4[0])

                single5 = self.arr5_test[i]
                single5 = np.reshape(single5, (1, -1))
                x5, index5 = self.SelectComponentBySingle(single5)
                reco5 = self.sess.run(self.recoArr[index5], feed_dict={self.inputs: x5})
                arr5.append(reco5[0])
                realarr5.append(x5[0])
            ...
            realarr1 = np.array(realarr1)
            realarr2 = np.array(realarr2)
            realarr3 = np.array(realarr3)
            realarr4 = np.array(realarr4)
            realarr5 = np.array(realarr5)

            realarr1 = np.reshape(realarr1,(-1,28,28,1))
            realarr2 = np.reshape(realarr2,(-1,28,28,1))
            realarr3 = np.reshape(realarr3,(-1,28,28,1))
            realarr4 = np.reshape(realarr4,(-1,28,28,1))
            realarr5 = np.reshape(realarr5,(-1,28,28,1))

            ims_cv(os.path.join("results/", 'HSIC_MNIST_Reco1.png'),merge2(arr1, [8, 8]))
            ims_cv(os.path.join("results/", 'HSIC_MNIST_Reco2.png'),merge2(arr2, [8, 8]))
            ims_cv(os.path.join("results/", 'HSIC_MNIST_Reco3.png'),merge2(arr3, [8, 8]))
            ims_cv(os.path.join("results/", 'HSIC_MNIST_Reco4.png'),merge2(arr4, [8, 8]))
            ims_cv(os.path.join("results/", 'HSIC_MNIST_Reco5.png'),merge2(arr5, [8, 8]))

            ims_cv(os.path.join("results/", 'HSIC_MNIST_Real1.png'),merge2(realarr1, [8, 8]))
            ims_cv(os.path.join("results/", 'HSIC_MNIST_Real2.png'),merge2(realarr2, [8, 8]))
            ims_cv(os.path.join("results/", 'HSIC_MNIST_Real3.png'),merge2(realarr3, [8, 8]))
            ims_cv(os.path.join("results/", 'HSIC_MNIST_Real4.png'),merge2(realarr4, [8, 8]))
            ims_cv(os.path.join("results/", 'HSIC_MNIST_Real5.png'),merge2(realarr5, [8, 8]))

            for i in range(self.componentCount):
                batch_z = np.random.uniform(-1, 1, [self.batch_size, self.z_dim]).astype(np.float32)
                gan = self.sess.run(self.GeneratorArr[i],feed_dict={self.z:batch_z})
                str1 = "HSIC_MNIST_GAN_Expert" + str(i) + ".png"
                gan = np.reshape(gan,(self.batch_size,28,28,1))
                ims_cv(os.path.join("results/", str1), merge2(gan, [8, 8]))

            '''
            arr1 = np.array(realclassArr).astype('str')
            myThirdName = "results/EEM_RealClass_MNIST_Threshold385.txt"
            ...
            myThirdName = "results/EEM_ComponentCount_MNIST_Threshold385.txt"
            f = open(myThirdName, "w", encoding="utf-8")
            for i in range(np.shape(arr1)[0]):
                f.writelines(arr1[i])
                f.writelines('\n')
