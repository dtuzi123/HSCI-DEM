...
class LifeLone_MNIST(object):
    ...
    def Calculate_Accuracy_ByBatch(self, testX, testY):
        totalCount = np.shape(testX)[0]
        totalCount = int(totalCount / self.data_stream_batch)

        myPro = []
        for i in range(totalCount):
            batch1 = testX[i * self.data_stream_batch:(i + 1) * self.data_stream_batch]
            batchy = testY[i * self.data_stream_batch:(i + 1) * self.data_stream_batch]

            x, index = self.SelectComponentByBatch(batch1)
            pred = self.sess.run(self.TeacherPredictions[index], feed_dict={self.inputs: x})

            for j in range(self.data_stream_batch):
                myPro.append(pred[j])
    ...
    def train(self):
        ...
        with tf.Session(config=config) as sess:
            ...
            for t1 in range(int(np.shape(self.totalSet)[0] / self.data_stream_batch)):
                ...
                if np.shape(pastArr)[0] == 0:
                    pastArr = newX
                    pastLabelArr = newXLabel
                else:
                    pastArr = np.concatenate((pastArr, newX), axis=0)
                    pastLabelArr = np.concatenate((pastLabelArr, newXLabel), axis=0)
                ...
                print(np.shape(self.FixedMemoryLabel))
                ...
                if np.shape(self.GeneratorArr)[0] == 1:
                    ...
                    for epoch in range(epochs):
                        ...
                        for idx in range(myCount):
                            ...
                            _, _ = self.sess.run(
                                [self.VAEOptimArr[self.componentCount - 2], self.VAEOptimArr[self.componentCount - 1]],
                    ...
                    for epoch in range(epochs):
                        ...
                        for idx in range(myCount):
                            ...
                            _, _ = self.sess.run(
                                [self.VAEOptimArr[self.componentCount - 1],
                                 self.ClassifierOptimArr[self.componentCount - 1]],
                                feed_dict={self.inputs: batchImages, self.label: batchLabels})
                ...
                myacc = self.Calculate_Accuracy_ByBatch(pastArr, pastLabelArr)
                totalArr.append(myacc)
            ...
            arr1 = np.array(totalArr).astype('str')
            myThirdName = "results/HSIC_theroem2.txt"
